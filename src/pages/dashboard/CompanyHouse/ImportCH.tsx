...
const ImportCH: React.FC = () => {
    ...
    const { data: stats, isLoading: isLoadingStats } = useCompanyHouseStats();
    const { mutate: startImport, isPending: isImporting } = useCompanyHouseImport();
    ...
    const fetchProgress = async () => {
        try {
            const response = await axiosInstance.get('/company-house/import-progress');
            if (response.data.success) {
                ...
                if (newProgress.isComplete || !newProgress.isRunning) {
                    // Ensure we mark progress as complete locally so UI stops spinner
                    setProgress(prev => ({ ...newProgress, isComplete: true }));
                    setIsPolling(false);
                    if (pollingInterval.current) {
                        clearInterval(pollingInterval.current);
            ...
            // Mark progress complete locally so UI doesn't keep showing spinner
