...
import { usePropPriceStats, usePropPriceImport, usePropPriceImportFiles, useUploadPropPriceFiles } from '@/api/propPrice';
...
const PropPriceImport = () => {
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
  const [uploadProgress, setUploadProgress] = useState(0);

  const { data: stats, isLoading: isLoadingStats, refetch: refetchStats } = usePropPriceStats();
  const { mutate: startImport, isPending: isImporting } = usePropPriceImport();
  const {
    data: importFiles,
    isLoading: isLoadingImportFiles,
    refetch: refetchImportFiles
  } = usePropPriceImportFiles();
  const { mutateAsync: uploadSingleFile, isPending: isUploading } = useUploadPropPriceFiles();
  ...
  const {
    ...
    progressEndpoint: '/prop-price/import-progress',
  ...
  const handleUploadFiles = async () => {
    ...
    if (selectedFiles.length > MAX_UPLOAD_FILES) {
      toast.error(`Maximum ${MAX_UPLOAD_FILES} files are allowed.`);
      return;
    }
    ...
    addLog({
      type: 'success',
      message: `Uploaded ${uploadedCount} file(s) to Prop Price import directory.`
    });
  ...
  const handleImport = () => {
    ...
    startImport(undefined, {
      onSuccess: () => {
        ...
        addLog({
          type: 'success',
          message: 'Starting Prop Price import process...'
        });
      },
      onError: (error: any) => {
        addLog({
  ...
  return (
    <div className="p-5 mx-auto">
      <h2 className="text-2xl font-bold mb-4">
        Prop Price Import - {isLoadingStats ? 'Loading...' : stats?.propPriceCount?.toLocaleString() || 0}
      </h2>

      <div className="mb-4 text-sm text-muted-foreground">
        <p>Prop Price Count: {stats?.propPriceCount?.toLocaleString() || 0}</p>
      </div>

      <FileUploadInstructions
        uploadPath={CSV_UPLOAD_PATHS.PROP_PRICE_IMPORTS}
        formatDescription="Upload one or multiple CSV files directly from this page, then click Start Import."
        additionalInfo="Expected columns: A unique_id, B price_paid, C deed_date, D postcode, E property_type, F new_build, G estate_type, H saon, I paon, J street. Display format: deed_date as YY/MM/DD and price_paid as £9,999,999."
      />
      ...
      <div className="mb-6 border border-gray-200 dark:border-gray-700 rounded-md p-4 bg-white dark:bg-gray-900/40">
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Upload limits: up to <span className="font-semibold text-foreground">{MAX_UPLOAD_FILES} files</span> per upload, <span className="font-semibold text-foreground">{MAX_UPLOAD_SIZE_MB} MB</span> per file.
        </p>
      ...
      <ImportProgress progress={progress} logs={logs} title="Prop Price Import Progress" />
    </div>
  );
};

export default PropPriceImport;
